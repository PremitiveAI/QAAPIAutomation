...
class ApiTestReports(Base):
    __tablename__ = "tbl_api_test_reports"

    id = Column(Integer, primary_key=True, index=True)
    collection_id = Column(Integer, ForeignKey("tbl_collections.id", ondelete="CASCADE"))
    test_id = Column(Integer, ForeignKey("tbl_test_reports.id", ondelete="CASCADE"))
    apiId = Column(Integer, ForeignKey("tbl_api_endpoints.id", ondelete="CASCADE"))

    test_report_file = Column(Text)
